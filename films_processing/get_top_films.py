...
from text_proccesing.unique_counter import count_word
...
num_rating = float(input('Введіть значення рейтингу фільма: '))


def des_rating(value):
    tit_list = []
    for k in imdb_rank:
        if float(k['rating']) >= value:
            tit_list.append(k['description'])
    return choice(tit_list)


print(des_rating(num_rating))
count_word(des_rating(num_rating))