# Завдання 1
cities = (('Ivano-Frankivsk', 'Ukraine'), ('Jakarta', 'Indonesia'),
          ('Lagos', 'Nigeria'), ('Beijing', 'China'), ('Los Angeles', '	United States'),
          ('Rivne', 'Ukraine'), ('Sumy', 'Ukraine'),
          ('Lviv', 'Ukraine'), ('Odessa', 'Ukraine'), ('Sumy', 'Ukraine'),
...
only_ua_cities = filter(only_ua, cities)
print(tuple(only_ua_cities))
...
second_element = lambda score: score[1]
high_to_low = sorted(grade, key = second_element, reverse = True)
print(high_to_low)
...
# Завдання 4
# Створимо нову зміну, яка буде зберігати лише назву фільма і рейтинг.
title_rating = []
for i in imdb_rank:
    title_rating.append({'title': i['title'], 'rating': i['rating']})

# Відфільтруємо фільми рейтинг яких вище 8.5.
high_rating = lambda value: float(value['rating']) > 8.5
high_filter = list(filter(high_rating, title_rating))
print(f'Завдання #4: {high_filter}')

# Відсортуємо за алфавітом по назві фільму.
film_name = lambda value: value['title']
high_sorted = sorted(high_filter, key = film_name)
print(f'Завдання #4: {high_sorted}')
...
title = input('Завдання #5: Введіть назву фільма, трейлер якого ви хочете переглянути: ')
print(f'Завдання #5: Трейлер до фільму {title} можете переглянути за посиланням: {trailer_link(title)}')


# Завдання 6
def director_info(name):
    films_count = 0
    title_list = []
    info = {'Name': name,}
    for i in imdb_rank:
        if i['director'][0] == name:
            films_count += 1
            title_list.append({'title': i['title'], 'rating': i['rating']})
            info.update({'Films count': films_count, 'Title and rating film': title_list})
        elif name in str(i['writers']):
            films_count += 1
            title_list.append({'title': i['title'], 'rating': i['rating']})
            info.update({'Films count': films_count, 'Title and rating film': title_list})
    return info
...
name = input(f"Завдання #6: Введіть ім'я  режисера/сценариста, щоб отримати інформацію: ")
print(f'Завдання #6: {director_info(name)}')


# Завдання 7
# Створимо змінну в якій зберігаються жанри, якими користувач зможе скористатися для пошуку фільмів
title_genre = []
for i in imdb_rank:
    for j in i['genre']:
        title_genre.append(j)
title_genre = set(title_genre)


def film_search_by_genre(name):
    films = []
    for i in imdb_rank:
      if name in str(i['genre']):
          films.append(i['title'])
    return films

name = input(f"Завдання #7: Введіть жанр для пошуку фільмів: ")
print(f'Завдання #7: {film_search_by_genre(name)}')
